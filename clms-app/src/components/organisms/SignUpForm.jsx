...
export default function SignUpForm({ role, onRoleChange, onSubmit }) {
  ...
  const content = authContent[role].signup;
  ...
  return (
    <form onSubmit={submit} className="w-full space-y-5 rounded-[32px] border border-white/80 bg-white/92 p-7 shadow-[0_24px_80px_rgba(15,23,42,0.12)] backdrop-blur sm:p-8">
      ...
      <p className="text-sm text-text-tertiary">Already have an account? <Link to={`/login?role=${role}`} className="text-brand hover:text-brand-hover">Log in</Link></p>
    </form>
  );
}
